...
class Grapher(object):
    ...
    def compute_membership_score(self, ts_idx, current_timestamp):
        ts_date = datetime.strptime(self.id2ts[ts_idx], "%Y-%m-%d %H:%M:%S")
        delta = current_timestamp - ts_date
        time_interval = delta.days + delta.seconds / (3600 * 24)
        return 1 / (1 + time_interval)
